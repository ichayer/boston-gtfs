...
def generate_color_palette(n):
    basic_colors = ["red", "blue", "green", "orange", "purple", "cyan", "magenta", "yellow", "black", "brown"]
...
if __name__ == "__main__":
    sql = """
    SELECT *
    FROM segment_trip_stats_june
    WHERE number_of_trips = (SELECT MAX(number_of_trips) FROM segment_trip_stats_june);
    """
    ...
    for _, row in gdf.iterrows():
        fl.GeoJson(
            ...
            tooltip=fl.Tooltip(f"Desde: {row['from_stop_name']}<br>"
                               f"Hasta: {row['to_stop_name']}<br>"
                               f"Viajes: {row['number_of_trips']}<br>"
                               f"Rutas: {row['routes']}")
        ).add_to(segment_map)

    segment_map.save("top_segments_colored.html")
    print("Mapa guardado en 'top_segments_colored.html'")
