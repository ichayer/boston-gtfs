...
if __name__ == "__main__":
    ...
    feed = Feed(gtfs_file, start_date=analysis_date, end_date=analysis_date, time_windows=time_windows)
    segments_freq = feed.segments_freq

    # --- Filter hotspots (segments with at least m vehicles in the same time window) ---
    hotspots = segments_freq[(segments_freq["ntrips"] >= m) & (segments_freq["route_id"] != "ALL_LINES")].copy()
    ...
    if overlapping_routes:
        ...
        df_overlap.rename(columns={"route_1": "route_1_id", "route_2": "route_2_id"}, inplace=True)

        # --- Print overlapping route pairs ---
        print("\n🔁 Overlapping route pairs:")
        for _, row in df_overlap.iterrows():
            print(f"Route {row['route_1_id']} overlaps with Route {row['route_2_id']} (shared hotspots: {row['shared_hotspots']})")
    else:
    ...
    if all_shared_segments_windows:
        ...
        shared_df = pd.DataFrame(all_shared_segments_windows, columns=["segment_name", "window"])
